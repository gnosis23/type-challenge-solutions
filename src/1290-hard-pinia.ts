...
type GetState<T> = T extends () => any ? ReturnType<T> : {}
type GetGetter<T> = {
...
declare function defineStore<S, G, A>(store: {
  id: string,
  state: S,
  getters: G & ThisType<Readonly<GetState<S>> & GetGetter<G>>,
  actions: A & ThisType<GetState<S> & Readonly<GetGetter<G>> & A>,
}): GetState<S> & Readonly<GetGetter<G>> & A
