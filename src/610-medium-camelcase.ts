...
type CamelCase<S> = S extends `${infer A}-${infer B}`
  ? B extends ''
    ? `${A}-`
    : B extends Capitalize<B>
      ? `${A}-${CamelCase<B>}`
      : CamelCase<`${A}${Capitalize<B>}`>
  : S;

type Capitalize<S> = S extends `${infer A}${infer R}` ? `${Uppercase<A>}${R}` : S;

type t1 = CamelCase<'foo-Bar-Baz'>;
