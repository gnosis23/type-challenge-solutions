...
function printObject<NestedObject extends Record<any, any>, S extends string>(item: NestedObject, field: S) {
  let fields = field.split('.');
  let ret = item;
  fields.forEach(i => {
    ret = ret[i];
  })
  return ret as Get<NestedObject, S>;
}

let a = printObject({ a: 100 }, 'a');
let b = printObject({ foo: { bar: 'hello' } }, 'foo.bar');
